...
[estimated_parameter_Tikhonov,c]= CRFIEKF_Tikhonov(time, model, jac, H , Q, R, xe0, Pe0, D, d, alpha);
disp('Parameter estimation completed with Tikhonov regulation')

% [estimated_parameter_L1]= CRFIEKF_L1(time, model, jac, H , Q, R, xe0, Pe0, D, d, alpha);
% disp('Parameter estimation completed with L1 regulation')
% 
% [estimated_parameter_Comb]= CRFIEKF_Comb(time, model, jac, H , Q, R, xe0, Pe0, D, d, alpha);
% disp('Parameter estimation completed with Combined regulation')
% disp('Validation with Hypoxia WET lab data started... please wait...');

x_v_Tikhonov = Hypoxia_simulation(estimated_parameter_Tikhonov);
fprintf('Hypoxia simulation completed using estimated parameters by Tikhonov regularization...\n');
% x_v_L1 = Hypoxia_simulation(estimated_parameter_L1);
% fprintf('Hypoxia simulation completed using estimated parameters by L1 regularization...\n');
% x_v_Comb = Hypoxia_simulation(estimated_parameter_Comb);
% fprintf('Hypoxia simulation completed using estimated parameters by Combined regularization...\n');
% pause(2);
% disp('All Validation with Hypoxia WET lab data completed');
% disp('Generating plot...');

% Parameter_estimation_FIS_membership_vs_Kinoshita_plot(x_v_Tikhonov, x_v_L1, x_v_Comb);
Parameter_estimation_FIS_membership_vs_Kinoshita_plot(x_v_Tikhonov, x_v_Tikhonov, x_v_Tikhonov);
