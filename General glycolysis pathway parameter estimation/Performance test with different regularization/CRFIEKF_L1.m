function [M] = CRFIEKF_L1(time, model, jac, Hk , Q, R, xe0, Pe0, D, d, alpha)
...
for ii=2:5000
    ...
    % **8** If condition number > Threshold ==> replace xe with L1 Regularized xe    
%     P_rkf = Pe{ii};
%     x_rkf(:,1) = xe(:,ii);
%     H_rkf = ones(1,79);
%         
...
%     x_reg(:,ii) = x_rkf(:,j+1);
    ...
    if (con(ii) > 9.30e3)
        test =  (Hk'*R^(-1)*Hk + Pe{ii} + alpha*(abs(reg_mat))^(1/2))^(-1);
        K_trkf = test  * (Hk'*R^(-1));   
...
%%% Estimating results
for i= 1:47
%    M(i,1) = mean(x_reg(i+32,ii-200:ii-1));
