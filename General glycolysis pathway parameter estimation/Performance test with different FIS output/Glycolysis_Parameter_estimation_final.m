...
% Initial value (equilibrium point)
...
Measurement_vec = zeros(6,79);

Measurement_vec(1,12) = 1;
Measurement_vec(2,15) = 1;
Measurement_vec(3,1) = 1;
Measurement_vec(4,8) = 1;
Measurement_vec(5,16) = 1;
Measurement_vec(6,4) = 1;
...
[estimated_parameter_fis_6]= CRFIEKF_Tikhonov_FIS_6_output(time, model, jac, Measurement_vec , Q, R, xe0, Pe0, D, d, alpha);
disp('Parameter estimation completed with 6 FIS output')

Measurement_vec = zeros(5,79);
...
Measurement_vec(5,16) = 1;
R = eye(5)*0.001;
[estimated_parameter_fis_5]= CRFIEKF_Tikhonov_FIS_5_output(time, model, jac, Measurement_vec , Q, R, xe0, Pe0, D, d, alpha);
disp('Parameter estimation completed with 5 FIS output')

Measurement_vec = zeros(4,79);

Measurement_vec(1,12) = 1;
Measurement_vec(2,8) = 1;
Measurement_vec(3,16) = 1;
Measurement_vec(4,4) = 1;
R = eye(4)*0.001;
[estimated_parameter_fis_4]= CRFIEKF_Tikhonov_FIS_4_output(time, model, jac, Measurement_vec , Q, R, xe0, Pe0, D, d, alpha);
disp('Parameter estimation completed with 4 FIS output')
...
Measurement_vec(6,4) = 1;
R = eye(6)*0.001;
[estimated_parameter_without_reg]= CRFIEKF_run_without_regularization(time, model, jac, Measurement_vec , Q, R, xe0, Pe0, D, d, alpha);

[Estimated_parameter_HEKF] = Parameter_estimation_main_HEKF();

disp('Parameter estimation completed');
disp('Validation with Hypoxia WET lab data started... please wait...');

load('Hypoxia_WET_lab_validation_02_10_2018.mat')
x_v_fis_6 = Hypoxia_simlation(estimated_parameter_fis_6);
fprintf('Hypoxia simulation completed using estimated parameters by 6 FIS output...\n');
x_v_fis_5 = Hypoxia_simlation(estimated_parameter_fis_5);
fprintf('Hypoxia simulation completed using estimated parameters by 5 FIS output...\n');
x_v_fis_4 = Hypoxia_simlation(estimated_parameter_fis_4);
fprintf('Hypoxia simulation completed using estimated parameters by 4 FIS output...\n');
x_v_without_reg = Hypoxia_simlation(estimated_parameter_without_reg);
fprintf('Hypoxia simulation completed using estimated parameters without regularization...\n');
x_v_HEKF = Hypoxia_simlation(Estimated_parameter_HEKF);
fprintf('Hypoxia simulation completed using estimated parameters by HEKF...\n');
pause(2);
disp('Validation with Hypoxia WET lab data completed');
disp('Generating plot...');

Parameter_estimation_by_6_vs_5_vs_4_FIS_op_vs_Kinoshita_plot(x_v_fis_6, x_v_fis_5, x_v_fis_4);
Parameter_estimation_by_6_vs_Kinoshita_vs_plot(x_v_fis_6, x_v_HEKF);
