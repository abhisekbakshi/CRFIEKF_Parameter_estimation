function [M] = IL6_jak_STAT3_CRFIEKF_run(time, model, jac, Hk , Q, R, xe0, Pe0, D, d, alpha)
...
fis_gp130_SOCS3_pRComplex = readfis('fuzzy_rule_gp130_SOCS3_pRComplex');
...
for ii=2:5000
    ...
    Pp=IL6_jak_STAT3_integration_cov_mat(Fk,Q,[time(ii-1) time(ii)],Pe{ii-1});
    Pp=Pp{numel(Pp)};
    ...
    observed(1,ii-1) = evalfis(fis_gp130_SOCS3_pRComplex, [xp(2,1) xp(6,1)],options);
%     observed(2,ii-1) = evalfis(fis_IL6R_Rcomplex_IL6_IL6R, [x7 x8],options);
    
    % **7** Incorporate fuzzy measurement to get a posteriori estimate of current prediction 
    xe(:,ii)=xp+Lk*(observed(1,ii-1) - Hk*xp);
    ...
    if (con(ii) > 9.30e3)
        ...
        x_trkf = xp + K_trkf*(observed(1,ii-1) - Hk*xp);
